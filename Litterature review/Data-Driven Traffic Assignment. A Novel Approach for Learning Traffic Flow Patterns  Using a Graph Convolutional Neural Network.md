...
It anserws this question : “How long does it take to go from node _i_ to node _j_ when there’s no traffic (free-flow condition)?
		![[images/Pasted image 20251018115741.png]]
...
3.  **Modele Architecture**
	...
	**Layer 3:** Aggregates all flows per link to get final traffic flow estimates.
	...
	- while the **graph convolution filter structure gθ and the **adjacency matrix (Aw)** come from the **network topology** (they are fixed, not learned).
...
The study proposes a **data-driven approach to the traffic assignment problem** using a **graph convolutional neural network (GCNN)** that learns how traffic flows propagate through a transportation network based on variations in origin-destination (OD) demand. Instead of relying on explicit behavioral models or iterative equilibrium computations, the model **learns a routing (diffusion) matrix** that represents how demand spreads from origins to destinations across connected links. Through experiments on the **Sioux Falls** and **East Massachusetts** networks, the GCNN accurately reproduces user equilibrium (UE) flows with less than **2% mean absolute error**, demonstrating both **high accuracy and computational efficiency**. Once trained, the model can instantly predict network-level traffic flows, making it suitable for **large-scale applications**. Moreover, the framework is **fully data-driven**, requiring no assumptions about user behavior, thus improving stability and reliability. The authors suggest that future research should extend this approach to **dynamic traffic assignment** by incorporating temporal flow variations and developing **better demand data** through augmentation or emerging data sources such as **mobile phones, GPS, and connected vehicle data**. This work marks a foundational step toward **data-driven network modeling** in the era of big transportation data.

- It was **not validated on real-world traffic data**, due to the lack of high-resolution OD datasets.
    
- It **did not test** robustness under **missing, noisy, or incomplete data**.
    
- The model was **not evaluated on unseen OD scenarios or dynamic conditions**, meaning generalization to real-world variability is still unproven